(function () {
  ...
  const staticBanner = document.getElementById("static-demo-banner");
  ...
  let panelHeights = { ...DEFAULT_PANEL_HEIGHTS };
  let detailsColLeftPct = 54.5;
  ...
  function renderBoardValue(p) {
    const key = p.key != null ? String(p.key) : "";
    const expandId = "board:" + key;
    const summary = payloadSummary(p.value) || "—";
    if (!valueNeedsExpand(p.value)) {
      return `<code class="value-summary">${esc(summary)}</code>`;
    }
    const openAttr = boardExpanded.has(expandId) ? " open" : "";
    return (
      `<details class="value-details"${openAttr} data-expand-key="${esc(expandId)}">` +
      `<summary><code class="value-summary">${esc(summary)}</code></summary>` +
      `<pre class="json-pretty scroll-themed">${esc(prettyJson(p.value))}</pre>` +
      `</details>`
    );
  }
  ...
  function panelScrollHosts(panelEl) {
    return panelEl.querySelectorAll(
      ".table-wrap, .event-list, .file-grid, .canvas-body, .ops-grid"
    );
  }

  function capturePanelScroll(panelEl) {
    const hosts = [];
    panelScrollHosts(panelEl).forEach((el, i) => {
      hosts.push({ i, top: el.scrollTop, left: el.scrollLeft });
    });
    const byKey = {};
    panelEl.querySelectorAll(".canvas-card").forEach((card) => {
      const key = card.getAttribute("data-key");
      const body = card.querySelector(".canvas-body");
      if (key && body) {
        byKey[key] = { top: body.scrollTop, left: body.scrollLeft };
      }
    });
    return { hosts, byKey };
  }

  function restorePanelScroll(panelEl, saved) {
    if (!saved) return;
    const list = panelScrollHosts(panelEl);
    (saved.hosts || []).forEach((s) => {
      const el = list[s.i];
      if (el) {
        el.scrollTop = s.top;
    ...
    panelEl.querySelectorAll(".canvas-card").forEach((card) => {
      const key = card.getAttribute("data-key");
      const body = card.querySelector(".canvas-body");
      const prev = key && saved.byKey ? saved.byKey[key] : null;
      if (body && prev) {
  ...
  function applyPanelHeights() {
    Object.keys(panelHeights).forEach((id) => {
      const h = panelHeights[id];
      if (h == null || !Number.isFinite(h)) return;
      const panel = document.getElementById(id);
      if (!panel) return;
      const hosts = panel.querySelectorAll(".table-wrap, .event-list, .file-grid");
      if (hosts.length) {
        hosts.forEach((el) => {
  ...
  function setPanelHtml(id, html, fingerprint) {
    const el = document.getElementById(id);
    if (!el) return false;
    if (panelFp.get(id) === fingerprint) return false;
    const saved = capturePanelScroll(el);
    el.innerHTML = html;
    panelFp.set(id, fingerprint);
    requestAnimationFrame(() => {
      restorePanelScroll(el, saved);
      applyPanelHeights();
    });
    return true;
  }

  function persistLayoutCols() {
    try {
      localStorage.setItem(LAYOUT_COLS_KEY, JSON.stringify({ left: detailsColLeftPct }));
    } catch {
      /* ignore */
    }
  }
  ...
  function persistLayoutHeights() {
    try {
      localStorage.setItem(LAYOUT_HEIGHTS_KEY, JSON.stringify(panelHeights));
    } catch {
      /* ignore */
    }
  }

  function applyDetailsCols(leftPct) {
    const clamped = Math.max(20, Math.min(80, leftPct));
    detailsColLeftPct = clamped;
    const el = document.getElementById("view-details");
    if (!el) return;
    el.style.setProperty("--details-col-left", clamped + "%");
    el.style.setProperty("--details-col-right", 100 - clamped + "%");
    el.style.setProperty("--details-split-pct", clamped + "%");
  }

  function loadLayoutPrefs() {
    try {
      const cols = JSON.parse(localStorage.getItem(LAYOUT_COLS_KEY) || "null");
      if (cols && typeof cols.left === "number") {
        applyDetailsCols(cols.left);
      } else {
        applyDetailsCols(detailsColLeftPct);
      }
    } catch {
      applyDetailsCols(detailsColLeftPct);
    }
    try {
      const heights = JSON.parse(localStorage.getItem(LAYOUT_HEIGHTS_KEY) || "null");
      if (heights && typeof heights === "object") {
        panelHeights = { ...DEFAULT_PANEL_HEIGHTS, ...heights };
      }
    } catch {
      /* ignore */
    }
  ...
  function isStaticDemo(d) {
    return !!(d && (d.static_demo === true || (d.demo && d.demo.kind === "static")));
  }
  ...
  function updateStaticDemoBanner(d) {
    staticDemo = isStaticDemo(d);
    ...
    if (staticDemo) {
      ...
      document.body.classList.remove("static-demo");
    }
  }
  ...
  async function loadDrawerLog(taskId) {
    ...
    if (!logEl) return;
    if (staticDemo) {
      logEl.textContent = "logs unavailable in static demo (observe-only).";
  ...
  document.getElementById("board")?.addEventListener("toggle", (e) => {
    const t = e.target;
    if (!(t instanceof HTMLDetailsElement)) return;
    const key = t.getAttribute("data-expand-key");
    if (!key) return;
    if (t.open) boardExpanded.add(key);
    else boardExpanded.delete(key);
  });
  ...
  function initDetailsLayout() {
    ...
    details.querySelectorAll("section.panel-section").forEach((sec) => {
      sec.addEventListener("focusin", () => {
        details.querySelectorAll("section.panel-section").forEach((s) => {
          s.classList.toggle("panel-focused", s === sec);
        });
      });
    });
    ...
    details.addEventListener("dblclick", (e) => {
      ...
      if (!head || !details.contains(head)) return;
      const section = head.closest("section[data-height-panel]");
      if (!section) return;
      const id = section.getAttribute("data-height-panel");
      if (!id) return;
      const base = DEFAULT_PANEL_HEIGHTS[id] || 260;
      const cur = panelHeights[id] || base;
      panelHeights[id] = cur >= base * 1.8 ? base : Math.round(base * 2.2);
      persistLayoutHeights();
      applyPanelHeights();
    });

    details.addEventListener("pointerdown", (e) => {
      const handle = e.target.closest(".panel-resize-y");
      if (!handle || !details.contains(handle)) return;
      const id = handle.getAttribute("data-height-panel");
      if (!id) return;
      e.preventDefault();
      const startY = e.clientY;
      const startH = panelHeights[id] || DEFAULT_PANEL_HEIGHTS[id] || 260;
      const onMove = (ev) => {
        const next = Math.max(120, Math.min(720, startH + (ev.clientY - startY)));
        panelHeights[id] = next;
        applyPanelHeights();
      };
      const onUp = () => {
    ...
    const splitter = document.getElementById("details-col-splitter");
    if (splitter) {
      ...
      splitter.addEventListener("keydown", (e) => {
        ...
        if (e.key === "ArrowLeft") {
          e.preventDefault();
          applyDetailsCols(detailsColLeftPct - 2);
          persistLayoutCols();
  ...
  function updateEmptyHint(d, hasAny) {
    ...
    if (isStaticDemo(d)) {
      emptyHint.classList.remove("daemon-down", "empty-ws");
      emptyHint.innerHTML =
        "Static demo snapshot is empty — regenerate with <code>npm run publish:demo</code>.";
      return;
    }
  ...
  function renderComputerFocus(d) {
    ...
    const actions = staticDemo
      ? `<div class="cf-actions cf-actions-static">` +
    ...
    body.innerHTML =
      ...
      `</div>` +
      actions;
    if (!staticDemo) wireComputerFocusButtons();
  }
  ...
  async function postPoiApi(path, body) {
    if (staticDemo) {
      throw new Error("static demo — mutate disabled");
    }
  ...
  function render(d) {
    ...
    const boardRows = board
      .map(
        (p) =>
          ...
          )}</td><td class="value-cell">${renderBoardValue(p)}</td></tr>`
      )
      .join("");
    setPanelHtml(
      "board",
    ...
    setPanelHtml(
      ...
      stableFp({
        ...
        leases: d.leases,
        triggers: d.triggers,
        blocked_pois: d.blocked_pois,
        trigger_failures: d.trigger_failures,
    ...
    setPanelHtml("canvases", renderCanvases(canvases), stableFp(canvases));
    setPanelHtml("events", renderEvents(events), stableFp(events));
    setPanelHtml("files", renderFiles(files), stableFp(files));
