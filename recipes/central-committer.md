...
```bash
porq poi table create commit_requests --cols message:string:poi --cols status:string
porq poi lock git head --holder committer --ttl 86400 --reason "central committer"
porq run --kind service --restart always --name committer --claim ".git/**" -- "…"
porq poi set commit_requests req-1 '{"message":"feat: x"}' --state pending --tier ephemeral
```
